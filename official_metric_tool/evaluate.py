...
parser.add_argument('--cls_n_data', default=0, type=int,
                    help='Max n data used in cls training (to save memory)')
...
parser.add_argument('--no_cache', action='store_true',
                    help='Disable inputs reference (e.g. when train cls)')
...
def extract_shower_and_energy(given_file, which):
    ...
    shower = given_file['showers'][:MAX_DATA]
    energy = given_file['incident_energies'][:MAX_DATA]
...
if __name__ == '__main__':
    ...
    # MAX_DATA = args.cls_n_data if args.cls_n_data else None
    ...
    reference_shower, reference_energy = extract_shower_and_energy(reference_file,
                                                                   which='reference')
    del reference_file
    # if not args.no_cache and os.path.exists(os.path.join(args.source_dir, args.reference_file_name + '.pkl')):
    ...
    if args.mode in ['all', 'avg-E']:
        ...
        for i in range(len(target_energies)-1):
            ...
            if target_energies[i] in reference_hlf.avg_shower_E:
                ...
                print(reference_shower.shape,which_showers.shape)
    ...
    if args.mode in ['cls-low', 'cls-high', 'cls-low-normed']: # 'all' not cls in all mode, run seperately
        if args.mode not in ['cls-low']:
